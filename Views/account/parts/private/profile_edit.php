	      	...
	      	<v-col class="white px-12 py-md-8 info-container mx-md-6" cols="12" md="7" v-if="profile_container">
	      		<v-row>
		            ...
		            <v-text-field type="text" name="first_name" v-model="profile.first_name" class="mt-3" :rules="validations.nameRules" outlined></v-text-field>
		          </v-col>

		          <v-col cols="12" md="6">
		            <label class="body-1 font-weight-thin pl-1">Numele de familie</label>
		            <v-text-field type="text" name="last_name" v-model="profile.last_name" class="mt-3" :rules="validations.nameRules" outlined></v-text-field>
		          </v-col>

		          <v-col cols="12" md="6">
		            <label class="body-1 font-weight-thin pl-1">Email</label>
		            <v-text-field type="email" name="email" v-model="profile.email" class="mt-3" :rules="validations.emailRules" outlined></v-text-field>
		          </v-col>
		          ...
		          <v-col cols="12" md="6">
		            ...
		            <v-dialog ref="birthdate_dialog" v-model="birthdate_modal" :return-value.sync="profile.birthdate" width="20vw">
		              <template v-slot:activator="{ on, attrs }">
		                <v-text-field class="mt-3" v-model="profile.birthdate" append-icon="mdi-calendar" readonly v-bind="attrs" v-on="on" :rules="validations.birthdateRules" outlined></v-text-field>
		              </template>
		          ...
		          <v-col cols="12" md="6">
		            ...
		            <v-select class="mt-3" v-model="profile.gender" :items="genders" item-text="text" item-value="value" :rules="validations.genderRules" outlined></v-select>
		          </v-col>

		          <v-col cols="12" md="6">
		            <label class="body-1 font-weight-thin pl-1">Țara</label>
		            <v-select class="mt-3" v-model="profile.country_selected" :items="countries" item-text="name" item-value="id" v-on:change="filterStates" :rules="validations.countryRules" outlined></v-select>
		          </v-col>

		          <v-col cols="12" md="6">
		            <label class="body-1 font-weight-thin pl-1">Orașul</label>
		            <v-select class="mt-3" v-model="profile.state_selected" :items="country_states" item-text="name" item-value="id" v-on:change='getLocation' :rules="validations.countryStateRules" outlined></v-select>
		          </v-col>

		          <v-col cols="12" md="6">
		            <label class="body-1 font-weight-thin pl-1">Parola</label>
		            <v-text-field type="password" name="password" v-model="profile.password" class="mt-3" outlined></v-text-field>
		          </v-col>

		          <v-col cols="12" md="6">
		            <label class="body-1 font-weight-thin pl-1">Confirmați parola</label>
		            <v-text-field type="password" name="password_confirm" v-model="profile.password_confirm" class="mt-3" :rules="validations.passwordConfirmRules" outlined></v-text-field>
		          </v-col>
