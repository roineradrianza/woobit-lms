...
<template v-for="book, i in <?= !empty($object) ? $object : 'form' ?>.meta.experience.books">
    <v-col class="d-flex justify-end" cols="12" v-if="<?= !empty($object) ? $object : 'form' ?>.status < 0 && <?= !empty($object) ? $object : 'form' ?>.application_id < 0">
        <v-btn color="error" @click="removeItem(<?= !empty($object) ? $object : 'form' ?>.meta.experience.books, i)">Eliminați</v-btn>
    </v-col>
    ...
    <v-col cols="12" md="4">
        ...
        <v-text-field type="text" name="book_position" v-model="book.publisher" outlined reactive>
        </v-text-field>
    </v-col>

    <v-col cols="12" md="4">
        <label class="body-1 font-weight-thin pl-1">URL-ul proiectului</label>
        <v-text-field type="url" name="book_url" v-model="book.url" :rules="validations.urlRules" outlined reactive>
        </v-text-field>
    </v-col>
    ...
    <v-col cols="12">
        ...
        <v-textarea name="book_description" v-model="book.description" counter="2000"
            :rules="validations.descriptionRules" auto-grow outlined reactive>
        </v-textarea>
    </v-col>
