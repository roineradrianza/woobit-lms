...
<template v-for="project, i in <?= !empty($object) ? $object : 'form' ?>.meta.experience.projects">
    <v-col class="d-flex justify-end" cols="12" v-if="<?= !empty($object) ? $object : 'form' ?>.status < 0 && <?= !empty($object) ? $object : 'form' ?>.application_id < 0">
        <v-btn color="error" @click="removeItem(<?= !empty($object) ? $object : 'form' ?>.meta.experience.projects, i)">Eliminați</v-btn>
    </v-col>
