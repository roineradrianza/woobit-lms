...
<template v-for="work, i in form.meta.experience.work">
    <v-col class="d-flex justify-end" cols="12" v-if="form.hasOwnProperty('status') && parseInt(form.status) != 0">
        <v-btn color="error" @click="removeItem(form.meta.experience.work, i)">Eliminați</v-btn>
    </v-col>
    ...
    <v-col cols="12" md="4">
        ...
        <v-text-field type="text" name="work_location" v-model="work.location" outlined
            :disabled="form.hasOwnProperty('status') && !parseInt(form.status)" reactive>
        </v-text-field>
    </v-col>
        ...
        <v-row align="center">
            <v-col class="d-flex justify-center" cols="12" md="4">
                <v-checkbox v-model="work.currently_active" label="În prezent dețin această poziție" :true-value="1"
                    :false-value="0" :disabled="form.hasOwnProperty('status') && !parseInt(form.status)" reactive></v-checkbox>
            </v-col>
    ...
    <v-col cols="12">
        ...
        <v-textarea name="work_description" v-model="work.description" :rules="validations.descriptionRules"
            counter="2000" outlined :disabled="form.hasOwnProperty('status') && !parseInt(form.status)" reactive>
        </v-textarea>
    </v-col>
