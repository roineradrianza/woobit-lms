...
<template v-for="course, i in form.meta.courses">
    <v-col class="d-flex justify-end" cols="12" v-if="form.hasOwnProperty('status') && parseInt(form.status) != 0">
        <v-btn color="error" @click="removeItem(form.meta.courses, i)">Eliminați</v-btn>
    </v-col>

    <v-col cols="12" md="4">
        <label class="body-1 font-weight-thin pl-1">Titlul cursului</label>
        <v-text-field type="text" name="course_title" v-model="course.title" :rules="validations.requiredRules" outlined :disabled="form.hasOwnProperty('status') && !parseInt(form.status)" :disabled="form.hasOwnProperty('status') && !parseInt(form.status)"
            reactive>
        </v-text-field>
    </v-col>
    ...
    <v-col cols="12">
        ...
        <v-textarea name="course_description" v-model="course.description" :rules="validations.descriptionRules"
            counter="2000" outlined :disabled="form.hasOwnProperty('status') && !parseInt(form.status)" :disabled="form.hasOwnProperty('status') && !parseInt(form.status)" reactive>
        </v-textarea>
    </v-col>
