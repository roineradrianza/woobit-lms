    ...
    <v-row>
        ...
        <v-col cols="12">
            <v-row>
                ...
                <v-col cols="12" v-if="form.meta.interests.length > 0">
                    <template v-for="item, i in form.meta.interests">
                        <v-chip class="ma-2" color="secondary"
                            v-if="form.hasOwnProperty('status') && parseInt(form.status) != 0" close
                            close-icon="mdi-delete" @click:close="removeItem(form.meta.interests, i)">
                            {{ item }}
                        </v-chip>
                ...
                <v-col cols="12" md="4"
                    v-if="form.meta.interests.length <= 15 && form.hasOwnProperty('status') && parseInt(form.status) != 0">
                    <v-text-field type="text" name="interes" label="Interes" v-model="interest" class="mt-3"
                        ...
                        <template #append>
                            <v-btn class="mt-n4 mr-n3 py-7" color="primary" @click="form.meta.interests.push(interest)"
                                :disabled="interest == ''">
                                Adăugați
                            </v-btn>
                        </template>
                ...
                <v-col cols="12">
                    <v-row>
                        <?php echo new Controller\Template('become-teacher/partials/courses') ?>
                    </v-row>
                </v-col>
            </v-row>
        </v-col>
