<v-sheet class="mb-6" elevation="2" rounded class="mx-auto" v-for="item in comments"
    v-if="item.comment_type == 'question'">
    <v-col class="px-4" cols="12">
        <v-avatar :color="item.gender == 'M' ? 'primary' : '#e70f66'" size="40">
            <v-icon color="white" size="35">
                {{ item.gender == 'M' ? 'mdi-face-man' : 'mdi-face-woman' }}
            </v-icon>
        </v-avatar>
        ...
        <v-row class="d-flex align-center">
            ...
            <v-col class="d-flex justify-end" cols="6">
                <v-btn color="secondary" @click="item.answer_box = true" text>
            ...
            <v-row class="px-4">
                <v-col cols="12" v-if="item.edit_comment_box == true">
                    <vue-editor :id="'question_edit_editor' + item.lesson_comment_id" class="mt-3 fl-text-input"
                        v-model="item.edited_comment" :editor-toolbar="customToolbar" placeholder="Scrieți întrebarea dvs. aici">
                    </vue-editor>
                    <v-btn class="primary white--text my-4" @click="editComment(item)"
                        :loading="item.edit_comment_loading">Actualizare</v-btn>
                    <v-btn class="red white--text my-4" @click="item.edit_comment_box = false">Închideți
                    </v-btn>
                </v-col>

                <v-col cols="12" v-if="item.answer_box == true">
                    <vue-editor :id="'answer_editor' + item.lesson_comment_id" class="mt-3 fl-text-input"
                        v-model="item.answer" :editor-toolbar="customToolbar" placeholder="Scrieți răspunsul dumneavoastră aici">
                    </vue-editor>
                    <v-btn class="primary white--text my-4" @click="saveAnswer(item)" :loading="item.answer_loading">
                        Actualizați răspunsul
                    </v-btn>
                    <v-btn class="red white--text my-4" @click="item.answer_box = false">Închideți</v-btn>
                </v-col>
                <v-col cols="12">
                        ...
                        <v-col class="px-4" cols="12">
                            ...
                            <v-row class="d-flex align-center">
                                ...
                                <v-col class="d-flex justify-end" cols="6">
                                    <template v-if="answer.editable">
                                        <v-btn color="primary" @click="item.edit_answer_box = true" text>
                                            <v-icon>mdi-pencil</v-icon>
                                        </v-btn>
                                        <v-btn color="red" @click="deleteAnswer(answer)" text>
                                            <v-icon>mdi-trash-can</v-icon>
                                        </v-btn>
                                    </template>
                                </v-col>
                            ...
                            <v-col cols="12" v-if="item.edit_answer_box == true">
                                ...
                                <v-btn class="primary white--text my-4" @click="editAnswer(item, answer)"
                                    ...
                                    Actualizare respuesta</v-btn>
                                <v-btn class="red white--text my-4" @click="item.edit_answer_box = false">Închideți
                                </v-btn>
                            </v-col>
                        </v-col>
                    </v-sheet>
