...
<v-col cols="12" md="12">
    <h3 class="text-h4 font-weight-bold">Materiale</h3>
</v-col>
    ...
    <v-row class="d-flex align-center">
        <v-col cols="6" class="d-flex align-center">
            <template v-if="resource.file !== undefined">
                <v-switch class="py-0 my-n6 v-normal-input" v-model="resource.preview_only" label="Sólo Vista Previa"
                    v-if="getExt(resource.file.name) == 'pdf'" false-value="0" true-value="1" dense inset>
                </v-switch>
            </template>
            <template v-else-if="resource.file === undefined && resource.url != ''">
                <v-switch class="py-0 my-0 v-normal-input" v-model="resource.preview_only" label="Sólo Vista Previa"
                    v-if="getExt(resource.url) == 'pdf'" false-value="0" true-value="1" dense inset>
                </v-switch>
            </template>
        </v-col>
