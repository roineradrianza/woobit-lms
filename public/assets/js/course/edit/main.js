...
let vm = new Vue({
  ...
  mounted() {
    this.course.course_id = this.$refs.course_container.$el.getAttribute('course-id')
    this.initialize()
    ...
    initialize() {
      ...
      this.$http.get(url).then(res => {
        if (res.body.length > 0) {
          ...
          this.course.new_featured_image = new File([], '')
          this.currentImage = res.body[0].featured_image
    ...
    save() {
      ...
      app.$http.post(url, data).then(res => {
        if (res.body.status == 'success') {
          if (res.body.data.featured_image != '') {
